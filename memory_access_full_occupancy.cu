...
__global__ void memory_access_full_occupancy_test(int N, double* a, double val)
{
  int i = blockIdx.x*blockDim.x + threadIdx.x;
...
void memory_access_full_occupancy_test()
{
  printf("\n\n***Running %s\n\n", __func__);
  ...
  outer_timer.begin();
  // TODO pick best launch for improving occupancy / perf
  memory_access_full_occupancy_test<<<nblocks, 32>>>(N, a, 1.0);
...
int main(int argc, char** argv)
{
  ...
  N = 512;
  memory_access_full_occupancy_test();
}
